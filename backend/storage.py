...
class ImageStore:
    ...
    def save_generation(self, request_payload: dict[str, Any], result: ImageGenerationResult) -> dict[str, Any]:
        image_id = str(uuid4())
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                INSERT INTO image_generations (
                    id, created_at, provider, model, prompt, revised_prompt,
    ...
    def list_generations(self, limit: int = 50, offset: int = 0) -> list[dict[str, Any]]:
        with self._connect() as conn:
            rows = conn.execute(
                """
                SELECT id, created_at, provider, model, prompt, revised_prompt, size, quality,
